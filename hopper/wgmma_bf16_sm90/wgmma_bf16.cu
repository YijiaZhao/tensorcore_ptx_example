/**
 * wgmma.mma_async 最小单元 — BF16 (sm_90a, H100/H20 专属)
 *
 * 指令: wgmma.mma_async.sync.aligned.m64n8k16.f32.bf16.bf16
...
__global__ void wgmma_bf16_kernel(float* D_out) {
...
__global__ void phase2_random_kernel(const uint8_t* A, const uint8_t* B, float* D) {
    ...
    asm volatile("{\n\t.reg .pred p;\n\tsetp.ne.b32 p, %6, 0;\n\t"
        "wgmma.fence.sync.aligned;\n\t"
        "wgmma.mma_async.sync.aligned.m64n8k16.f32.bf16.bf16 {%0,%1,%2,%3}, %4, %5, p, 1, 1, 0, 0;\n\t"
        "wgmma.commit_group.sync.aligned;\n\t"
        "wgmma.wait_group.sync.aligned 0;\n\t}\n"
        : "=f"(d0),"=f"(d1),"=f"(d2),"=f"(d3)
        : "l"(da), "l"(db), "r"(0u));
