...
__global__ void wgmma_tf32_kernel(float* D_out) {
    ...
    // A=B=tf32(1.0)=0x3F800000, 按4字节/元素存
...
static int phase2_random_vs_cpu(uint32_t seed) {
    const int K = 8, EBITS = 32, ROUNDS = 3;
...
int main() {
    ...
    printf("====== TF32 (wgmma.mma_async, sm_90a) ======\n");
    printf("A[64x8]=1.0 (smem desc), B[8x8]=1.0 (smem desc)\n");
    printf("M=%d N=%d K=%d, warpgroup=128线程, expect D=8.0\n\n", M, N, K);

    wgmma_tf32_kernel<<<1, THREADS, 8192>>>(d_out);
    cudaError_t e = cudaDeviceSynchronize();
    if (e) { printf("Error: %s\n", cudaGetErrorString(e)); }
    else {
        CHECK_CUDA(cudaMemcpy(h, d_out, 512 * sizeof(float), cudaMemcpyDeviceToHost));
        int pass = 0;
        for (int i = 0; i < 512; i++) if (h[i] == 8.0f) pass++;
        printf("Result: %d/512 correct (=8.0)\n", pass);
