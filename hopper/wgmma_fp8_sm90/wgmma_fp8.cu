/**
 * wgmma.mma_async 最小单元 — FP8 e4m3 (sm_90a, H100/H20 专属) — Hopper 唯一原生 fp8 路径(QGMMA); mma.sync fp8 在 sm_90 是 fp16 模拟
 *
 * 指令: wgmma.mma_async.sync.aligned.m64n8k32.f32.e4m3.e4m3
 *       {d0..d3}, desc_a, desc_b, scale_d(p), scale_a, scale_b  (fp8无trans参数)
 *
 ...
 *
 * Tile: M=64, N=8, K=32;  D[64×8] f32 = 128线程 × 4寄存器
...
__global__ void wgmma_fp8_kernel(float* D_out) {
    ...
    // A=B=e4m3(1.0)=0x38
    for (int i = tid; i < A_BYTES / 4; i += THREADS) smem_a[i] = 0x38383838u;
    for (int i = tid; i < B_BYTES / 4; i += THREADS) smem_b[i] = 0x38383838u;
    __syncthreads();
...
int main() {
    ...
    printf("====== FP8 e4m3 (wgmma.mma_async, sm_90a) ======\n");
    printf("A[64x32]=1.0 (smem desc), B[32x8]=1.0 (smem desc)\n");
    printf("M=%d N=%d K=%d, warpgroup=128线程, expect D=32.0\n\n", M, N, K);

    wgmma_fp8_kernel<<<1, THREADS, 8192>>>(d_out);
    cudaError_t e = cudaDeviceSynchronize();
    if (e) { printf("Error: %s\n", cudaGetErrorString(e)); }
    else {
        CHECK_CUDA(cudaMemcpy(h, d_out, 512 * sizeof(float), cudaMemcpyDeviceToHost));
        int pass = 0;
        for (int i = 0; i < 512; i++) if (h[i] == 32.0f) pass++;
        printf("Result: %d/512 correct (=32.0)\n", pass);
