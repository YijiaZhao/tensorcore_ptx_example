...
__global__ void wgmma_int8_kernel(int32_t* D_out) {
    ...
    // A=B=s8(1)=0x01, s32累加
    for (int i = tid; i < A_BYTES / 4; i += THREADS) smem_a[i] = 0x01010101u;
    for (int i = tid; i < B_BYTES / 4; i += THREADS) smem_b[i] = 0x01010101u;
    __syncthreads();
...
__global__ void phase2_random_kernel(const uint8_t* A, const uint8_t* B, float* D) {
    ...
    asm volatile("{\n\t.reg .pred p;\n\tsetp.ne.b32 p, %6, 0;\n\t"
        ...
        : "=r"(d0),"=r"(d1),"=r"(d2),"=r"(d3)
        : "l"(da), "l"(db), "r"(0u));
    D[phase2_d_pos(tid,0)]=(float)d0; D[phase2_d_pos(tid,1)]=(float)d1;
    D[phase2_d_pos(tid,2)]=(float)d2; D[phase2_d_pos(tid,3)]=(float)d3;
}
...
int main() {
    int32_t *d_out, h[512];
    CHECK_CUDA(cudaMalloc(&d_out, 512 * sizeof(int32_t)));
    CHECK_CUDA(cudaMemset(d_out, 0, 512 * sizeof(int32_t)));

    printf("====== INT8 s8 (wgmma.mma_async, sm_90a) ======\n");
    printf("A[64x32]=1 (smem desc), B[32x8]=1 (smem desc)\n");
    printf("M=%d N=%d K=%d, warpgroup=128线程, expect D=32\n\n", M, N, K);

    wgmma_int8_kernel<<<1, THREADS, 8192>>>(d_out);
    cudaError_t e = cudaDeviceSynchronize();
    if (e) { printf("Error: %s\n", cudaGetErrorString(e)); }
    else {
        CHECK_CUDA(cudaMemcpy(h, d_out, 512 * sizeof(int32_t), cudaMemcpyDeviceToHost));
        int pass = 0;
        for (int i = 0; i < 512; i++) if (h[i] == 32) pass++;
        printf("Result: %d/512 correct (=32)\n", pass);
