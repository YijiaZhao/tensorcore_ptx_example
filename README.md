...
- A/B 每元素独立随机；block scale 的 **scale 也随机**（范围由 fp32 精确性预算推出：
  ue8m0 七档 2^±3、ue4m3 十二档——超出则大数吃小数、`==` 失效）
- CPU 三重循环算参考，逐元素 `==` 精确比对（取值集设计保证任意累加顺序零舍入）
- tcgen05 的 D 读回布局用 **base-4 one-hot 探针**真机实测（行/列按 4 进制逐位，支持 N=256、
  cta_group::2 双 CTA），一致性探针过滤 TMEM 跨 kernel 残留——不依赖任何文档假设
- 结构变体同样随机驱动：ws 流水线直接喂随机 4 tile（K=256）；warp_count 对 1/2/4 warp
  各跑全套；TS（A 在 TMEM）用**提取法**——随机 st 后拿 64 轮 B one-hot 把硬件实际
