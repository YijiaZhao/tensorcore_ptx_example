/**
 * LDGSTS configurable multistage tcgen05.mma demo (sm_100a / sm_103a)
 *
 * Layout:
 ...
 *   nvcc -gencode arch=compute_103a,code=sm_103a -std=c++17 -O3 -o ws_tcgen05_nvfp4_ldgsts_pipeline ws_tcgen05_nvfp4_ldgsts_pipeline.cu
 * Compile (B100/B200 sm_100a):
 *   nvcc -gencode arch=compute_100a,code=sm_100a -std=c++17 -O3 -o ws_tcgen05_nvfp4_ldgsts_pipeline ws_tcgen05_nvfp4_ldgsts_pipeline.cu
 */
...
static_assert(STAGES >= 2 && STAGES <= 4, "PIPELINE_STAGES must be 2..4");
static_assert(STAGES <= K_ITERS, "STAGES must not exceed K_ITERS");
...
ws_tcgen05_ldgsts_pipeline_kernel(uint8_t const* __restrict__ gmem_A,
...
#if PIPELINE_VERBOSE
    if (tid == 0) {
        printf("LDGSTS ring: stages=%d, 1 producer + %d consumer warps (%d threads)\n", STAGES, CONS_WARPS, TOTAL_THREADS);
...
//   探针和随机数据都通过 ws_tcgen05_ldgsts_pipeline_kernel 本体(producer/consumer流水线)跑,
...
inline void run(uint8_t* dA, uint8_t* dB, float* dD) {
    ...
    cudaFuncSetAttribute(ws_tcgen05_ldgsts_pipeline_kernel,
        cudaFuncAttributeMaxDynamicSharedMemorySize, smem_bytes);
    ws_tcgen05_ldgsts_pipeline_kernel<<<1, TOTAL_THREADS, smem_bytes>>>(dA, dB, dD);
