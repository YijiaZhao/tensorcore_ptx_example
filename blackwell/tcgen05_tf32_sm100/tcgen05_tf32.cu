...
static const int EBITS = 32;  // P2EBITS_FIXED
static uint32_t phase2_enc_small(int v) {   // 编码 {0,1,2,3,4,6} → 本格式
    static const int      vals[] = {0, 1, 2, 3, 4, 6};
    static const uint32_t encs[] = {0x00000000, 0x3F800000, 0x40000000, 0x40400000, 0x40800000, 0x40C00000};
...
static int phase2_random_vs_cpu(uint32_t seed) {
    ...
    for (int r = 0; r < ROUNDS; r++) {
        ...
        memset(p2_hA, 0, A_BYTES); memset(p2_hB, 0, B_BYTES);
        fill_random(p2_hA, refA, M, K, TF32_SET, SETN(TF32_SET), EBITS, &seed);
        fill_random(p2_hB, refB, N, K, TF32_SET, SETN(TF32_SET), EBITS, &seed);
