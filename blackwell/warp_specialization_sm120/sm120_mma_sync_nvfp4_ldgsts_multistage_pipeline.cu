/**
 * SM120 NVFP4 LDGSTS multistage pipeline.
 *
 * CTA layout:
 *   warp 0   : producer, 32 lanes issue 16-byte cp.async transfers
...
__device__ __forceinline__ void copy_16b_async(void *destination_shared,
                                               const void *source_global) {
    asm volatile(
        "cp.async.cg.shared::cta.global [%0], [%1], 16;" ::"r"(shared_address(destination_shared)),
        "l"(source_global)
        : "memory");
}
...
__global__ void nvfp4_ldgsts_pipeline_kernel(const uint8_t *global_a, const uint8_t *global_b,
    ...
    if (warp_id == 0) {
        ...
        for (int k_iter = 0; k_iter < kKIters; ++k_iter) {
            ...
            for (int offset = lane_id * 16; offset < kABytesPerStage; offset += 32 * 16) {
                copy_16b_async(shared_a[stage] + offset,
                               global_a + k_iter * kABytesPerStage + offset);
            }
            for (int offset = lane_id * 16; offset < kBBytesPerStage; offset += 32 * 16) {
                copy_16b_async(shared_b[stage] + offset,
                               global_b + k_iter * kBBytesPerStage + offset);
            }
            asm volatile("cp.async.commit_group;" ::: "memory");
            asm volatile("cp.async.wait_group 0;" ::: "memory");
            asm volatile("bar.arrive %0, %1;" ::"r"(stage * 2), "n"(kThreads));
        }
        return;
    }
    ...
    for (int k_iter = 0; k_iter < kKIters; ++k_iter) {
        const int stage = k_iter % kStages;
        asm volatile("bar.sync %0, %1;" ::"r"(stage * 2), "n"(kThreads));
...
int main() {
    ...
    CHECK_CUDA(cudaFuncSetAttribute(nvfp4_ldgsts_pipeline_kernel,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize, shared_bytes));

    constexpr uint32_t kScaleOne = 0x38383838u;
    nvfp4_ldgsts_pipeline_kernel<<<1, kThreads, shared_bytes>>>(device_a, device_b, device_d,
                                                                kScaleOne, kScaleOne);
    ...
    std::printf("SM120 NVFP4 LDGSTS stages=%d: %s (%d/%d exact)\n", kStages,
                errors ? "FAIL" : "PASS", kOutputElements - errors, kOutputElements);
